...
The `home-routes` handler in the `dok-jam.routes.home` namespace
...
The `home-page` function will in turn call the `dok-jam.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `dok-jam.handler` namespace:
...
Request middleware functions are located under the `dok-jam.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `dok-jam.dev-middleware` namespace found in
